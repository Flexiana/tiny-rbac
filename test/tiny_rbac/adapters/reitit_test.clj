;(ns tiny-rbac.adapters.reitit-test
;  (:require
;    [clojure.test :refer :all]
;    [tiny-rbac.adapters.reitit :refer [is-allowed]]))
;
;(def custom-roles
...
;         (is-allowed (state-with-user guest) {:resource "items" :privilege :create})))
;  (is (= "Not ok 401"
;         (state-with-user guest)))
;  (is (= :all
;         (is-allowed (state-with-user guest) {:resource "items" :privilege :read})))
;  (is (= {:status 401 :body "Authorization error"}
